export function getMatrixArray(id) {
  ...
  const matrixArray = Array(rowSize)
    .fill()
    .map(() => Array(colSize).fill(0));
  for (let i = 0; i < rowSize; i++) {
    for (let j = 0; j < colSize; j++) {
      const matrixInput = matrix.querySelector(
...
export function getMatrixArrayAttributesAndContainer(id) {
  const matrix = document.getElementById(id);
  const matrixInputContainers = Array.from(
    matrix.getElementsByClassName("inputContainer"),
  );
...
export function setMatrixInputFrom2dArray(id, result) {
  const matrix = document.getElementById(id);
  const rowSize = result.length;
  const colSize = result[0].length;
  for (let i = 0; i < rowSize; i++) {
    for (let j = 0; j < colSize; j++) {
      const matrixInput = matrix.querySelector(
        `input[data-row="${i}"][data-col="${j}"]`,
      );
      matrixInput.value = result[i][j]
    }
  }
}
